...
Asosiy yo'l — kim-qachon-nima bosqichma-bosqich (wizard) so'raladi va oddiy
qoidalar bilan tahlil qilinadi (AI ISHLATILMAYDI), shuning uchun Gemini
kvotasi tugab qolsa ham bu funksiya 100% ishlayveradi.

Qulaylik uchun QO'SHIMCHA sifatida: agar owner hammasini (kim, qachon, nima)
bitta xabarda yozsa, avval `parse_schedule_text` bilan Gemini orqali tez
ajratishga harakat qilinadi; muvaffaqiyatsiz bo'lsa (kvota, tarmoq xatosi)
oddiy bosqichma-bosqich rejimga muammosiz qaytiladi — hech qachon shunchaki
xato berib to'xtab qolmaydi.

Yuborish vaqti JobQueue orqali rejalashtiriladi; bot qayta ishga tushganda
hali yuborilmagan xabarlar bazadan o'qib qayta rejalashtiriladi
...
async def parse_schedule_text(text: str) -> ScheduleParse:
    """Owner bitta xabarda hammasini (kim, qachon, nima) yozgan bo'lsa,
    Gemini bilan ajratishga harakat qiladi. Ixtiyoriy tezlashtirish qatlami —
    ishlamasa (LLMError) chaqiruvchi bosqichma-bosqich rejimga qaytadi."""
    now = local_now()
    system_prompt = (
        ...
        "yoki ismga '-ga'/'aka'/'opa' kabi qo'shimchalar qo'shilgan bo'lishi mumkin). "
        ...
        "QOIDALAR:\n"
        "- recipients: qabul qiluvchilarning FAQAT ISMLARI ro'yxati, qo'shimcha "
        "va sifatlarsiz (masalan \"Bahodirga\" -> \"Bahodir\", \"Abdulvahob "
        "akaga\" -> \"Abdulvahob\"). Bittagina bo'lsa ham ro'yxat qaytar.\n"
        ...
        "- understood: kimga, qachon va nima yozish kerakligining barchasini "
        "aniq tushungan bo'lsang true, aks holda false."
    )
